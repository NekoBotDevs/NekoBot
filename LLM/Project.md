...
此项目为一个基于 Python 的架构，旨在构建一个支持多聊天平台大模型 (LLM/TTL) 机器人的聊天机器人，并使用 Next.js 构建的前端仪表盘进行配置和展示。
...
*   由用户手动从前端仪表盘上传插件(必须要验证jwt密钥),然后后端开始处理zip插件,解压至./data/temp(如果没有则自动创建目录),随后解压至 **/data/plugins** 插件目录
...
LLM/TTL 服务商配置存储在./data/cmd_config.json中,且支持动态加载 使用NekoConfigManager 进行加载配置 配置包含 API 密钥、模型设置和超时等参数。
...
支持直接将Markdown/txt文件上传至服务器(前端仪表盘上传)或者直接在仪表盘中编辑提示词和人设
...
使用NekoConfigManager方法进行动态配置加载,

### mcp支持

存储时使用json格式存储，文件名为mcpserver.json,储存至./data内
...
*   初次登录需使用默认账户密码(均为nekobot)，并在登录后强制要求用户修改密码(前端)/[(用户也可一并修改)(用户名非必须)]
...
*   直接使用*python main.py*启动 不需要任何脚本,如要测试,请写入 **./test** 文件夹内
